...
#####################################################################################
from bimanlp.utils.errors import NGramErr

def ngrams(toksentence,n=1,njump=0,sb='<s>',se='</s>'):
    ...
    try:
        if type(toksentence) != list:
            raise NGramErr({"message":"Please input tokenized sentence!!", "tokenize":type(toksentence)})
        else:
            if len(toksentence) == 0:
                raise NGramErr({"message":"Len of tokenized sentence must be > 0", "tokenize":len(toksentence)})

            if n_gram > len(kata):
                print "Len N-Gram: %i => Len Sentence: %i"%( n_gram, len(kata))
                raise NGramErr({"message":"Len of N-Gram cannot be greater than len of word", \
